...
class KNN_OpponentModel(OpponentModel):
    def __init__(self, domain: Domain, k=5):
        super().__init__(domain)
        self.k = k

        # Create a KNeighborsRegressor for each issue
        self.knn_models = {
            i: KNeighborsRegressor(n_neighbors=self.k)
            for i, v in domain.getIssuesValues().items()
        }

    def update(self, bid: Bid, issue_estimator=None):
        print(f"updatating bid : {bid}")
        super().update(bid)

        # Update the kNN models for each issue
        for issue_id, knn_model in self.knn_models.items():
            value_counts = [
                issue_estimator.value_trackers[value].count
                for value in self.issue_estimators[issue_id].value_trackers.keys()
            ]
            print(f"value_counts : {value_counts}")
            # Check if there are enough data points to fit the model
            #if sum(value_counts) <= self.k:
            X = np.array(value_counts).reshape(-1, 1)
            y = np.array(list(self.issue_estimators[issue_id].value_trackers.keys()))
            print(f"X : {X} \n y : {y}")
            knn_model.fit(X, y)
            print(f"knn_model score :  {knn_model.score(X, y)}")
...
class ValueEstimator:
    ...
    def recalculate_utility(self, max_value_count: int, weight: float):
        if weight < 0.99:
            ...
            mod_max_value_count = ((max_value_count + 0.95) ** (1 - weight)) - 1
